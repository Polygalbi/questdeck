...
export default function Home() {
  ...
  const [timelineRowHeight, setTimelineRowHeight] = useState(108);
  ...
  return <main className="app-shell">
    ...
    <section className="workspace-main">
      ...
      {view === "timeline" && <div className="content schedule-content">
        ...
        <div className="timeline-toolbar"><label>{tr("Project", "프로젝트")}<select value={project} onChange={event => setProject(event.target.value)}><option value="All projects">{tr("All projects", "모든 프로젝트")}</option>{activeProjects.map(item => <option key={item.id} value={item.name}>{item.name}</option>)}</select></label><label>{tr("Sort", "정렬")}<select value={timelineSort} onChange={event => setTimelineSort(event.target.value as typeof timelineSort)}><option value="Due date">{tr("Due date", "마감일")}</option><option value="Status">{tr("Status", "상태")}</option><option value="Owner">{tr("Owner", "담당자")}</option><option value="Name">{tr("Name", "이름")}</option></select></label><label className="timeline-height-control">{tr("Row height", "행 높이")}<span><input type="range" min="76" max="150" step="2" value={timelineRowHeight} onChange={event => setTimelineRowHeight(Number(event.target.value))} aria-label={tr("Timeline row height", "타임라인 행 높이")} /><output>{timelineRowHeight}px</output></span></label><span><b>{timelineCards.length}</b> {tr("scheduled cards", "개 일정 카드")}</span>{project !== "All projects" && <button onClick={() => setProject("All projects")}>× {tr("Clear project", "프로젝트 필터 해제")}</button>}</div>
