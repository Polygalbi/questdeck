...
async function syncQuestdeck<T = { ok: boolean }>(action: string, payload: Record<string, unknown>, accessToken: string): Promise<T> {
  ...
  if ((action === "create_card" || action === "update_card") && card && Number.isSafeInteger(card.id)) {
    const collaboratorResponse = await fetch(`${SUPABASE_URL}/rest/v1/questdeck_cards?id=eq.${card.id}`, {
      method: "PATCH",
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${SUPABASE_URL}/rest/v1/questdeck_cards?select=id,title,description,tag,owner_initials,collaborator_initials,points,priority,color,status,due_label,due_date,start_date,archived,questdeck_projects(name)&order=id.asc`, { headers }).then(response => {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace-main">
      ...
      {view === "overview" && <div className="content">
        ...
        <div className="card-grid hand-grid">{filtered.filter(card => card.status !== "Done").slice(0, 3).map(card => <QuestCard card={card} onOpen={openCard} todoSummary={cardTodoSummary(card)} key={card.id}/>)}</div>
