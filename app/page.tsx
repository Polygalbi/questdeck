  ...
  return <main className="app-shell">
    ...
    <section className="workspace-main">
      ...
      {view === "quests" && <div className="content board-content">
        ...
        <div className="board">
          {(["Ready", "In progress", "Review", "Done"] as Status[]).map((status, index) => <section className="board-column" key={status}><header><span className={`status-dot s${index}`}/><h3>{statusLabel(status)}</h3><b>{filtered.filter(c => c.status === status).length}</b><button aria-label={`${status} options`}>•••</button></header><div className="column-cards">{filtered.filter(c => c.status === status).map(card => <QuestCard card={card} onOpen={setSelected} compact key={card.id}/>)}<button className="add-inline" onClick={() => setCreateOpen(true)}>＋ {tr("Add a card", "카드 추가")}</button></div></section>)}
        </div>
      </div>}
