...
export default function Home() {
  ...
  const [selectedMindmapEdgeId, setSelectedMindmapEdgeId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if ((event.key === "Delete" || event.key === "Backspace") && selectedMindmapEdgeId) { event.preventDefault(); disconnectMindmapEdge(selectedMindmapEdgeId); }
      else if ((event.key === "Delete" || event.key === "Backspace") && selectedMindmapNodeId) { event.preventDefault(); deleteMindmapNode(selectedMindmapNodeId); }
  ...
  }, [view, selectedMindmapNodeId, selectedMindmapEdgeId, mindmapNodes]);
  useEffect(() => {
  ...
  function connectMindmapNode(nodeId: string) {
    if (!linkingMindmapNodeId) { setLinkingMindmapNodeId(nodeId); setSelectedMindmapNodeId(nodeId); setSelectedMindmapEdgeId(null); return; }
    ...
    if (!duplicate) {
      ...
      setMindmapEdges(current => [...current, { id: edgeId, from: linkingMindmapNodeId, to: nodeId }]);
      setSelectedMindmapEdgeId(edgeId);
      markMindmapDirty();
    } else setToast(tr("These ideas are already connected", "이미 연결된 아이디어입니다"));
    setSelectedMindmapNodeId(nodeId);
    setLinkingMindmapNodeId(null);
  }

  function disconnectMindmapEdge(edgeId: string) {
    if (!mindmapEdges.some(edge => edge.id === edgeId)) return;
    setMindmapEdges(current => current.filter(edge => edge.id !== edgeId));
    setSelectedMindmapEdgeId(current => current === edgeId ? null : current);
    markMindmapDirty();
    setToast(tr("Connection removed", "연결을 해제했습니다"));
  }
  ...
  const selectedMindmapConnections = selectedMindmapNode ? mindmapEdges.filter(edge => edge.from === selectedMindmapNode.id || edge.to === selectedMindmapNode.id).map(edge => ({ edge, node: mindmapNodes.find(node => node.id === (edge.from === selectedMindmapNode.id ? edge.to : edge.from)) })).filter(item => item.node) : [];
  ...
  return <main className="app-shell">
    <aside className={`sidebar ${mobileNavOpen ? "mobile-open" : ""}`}>
      ...
      <div className="sidebar-bottom"><button className="nav-item"><span>?</span> {tr("Help & shortcuts", "도움말 및 단축키")}</button><button className="nav-item sidebar-font-button" onClick={() => { setFontPickerOpen(true); setMobileNavOpen(false); }} aria-haspopup="dialog"><span>Aa</span> {tr("Screen font", "화면 글꼴")}<i>{uiFonts.find(font => font.id === uiFont)?.[language === "ko" ? "nameKo" : "name"]}</i></button><div className="profile-menu-wrap">{profileMenuOpen && <div className="profile-menu" role="menu"><button role="menuitem" onClick={() => { setView("account"); setProfileMenuOpen(false); setMobileNavOpen(false); }}>◉ <span>{tr("My account", "내 계정")}</span></button><button role="menuitem" onClick={() => { if (!session) { setAuthOpen(true); setProfileMenuOpen(false); return; } setNameEditorOpen(true); setProfileMenuOpen(false); }}>✎ <span>{tr("Edit name", "이름 수정")}</span></button><button role="menuitem" onClick={() => { setProfileMenuOpen(false); session ? void supabase.auth.signOut() : setAuthOpen(true); }}>{session ? "↪ " + tr("Sign out", "로그아웃") : "↪ " + tr("Sign in", "로그인")}</button></div>}<button className="profile profile-button" onClick={() => setProfileMenuOpen(current => !current)} aria-haspopup="menu" aria-expanded={profileMenuOpen}><span>{accountInitials}</span><div><b>{accountName}</b><small>{currentRole ?? tr("No workspace role", "워크스페이스 역할 없음")}</small></div><i>{profileMenuOpen ? "⌄" : "›"}</i></button></div></div>
    </aside>
    {fontPickerOpen && <div className="modal-backdrop" onMouseDown={() => setFontPickerOpen(false)}><section className="modal create-modal font-picker-modal" onMouseDown={event => event.stopPropagation()} role="dialog" aria-modal="true" aria-label={tr("Screen font", "화면 글꼴")}><header><div><small>{tr("APPEARANCE", "화면 설정")}</small><h2>{tr("Screen font", "화면 글꼴")}</h2><p>{tr("Choose a comfortable font for every Questdeck page.", "Questdeck의 모든 화면에 사용할 편안한 글꼴을 선택하세요.")}</p></div><button onClick={() => setFontPickerOpen(false)} aria-label={tr("Close font settings", "글꼴 설정 닫기")}>×</button></header><div className="font-option-grid">{uiFonts.map(font => <button type="button" data-font={font.id} className={uiFont === font.id ? "selected" : ""} onClick={() => void chooseUiFont(font.id)} aria-pressed={uiFont === font.id} key={font.id}><b>{language === "ko" ? font.nameKo : font.name}</b><span>{tr("Plan today · Ship tomorrow 0123", "가나다라마바사 · 하루 계획 0123")}</span>{uiFont === font.id && <i>✓</i>}</button>)}</div><div className="font-live-preview"><small>{tr("CURRENT FONT PREVIEW", "현재 글꼴 미리보기")}</small><p>{tr("Organize today’s work one card at a time. 0123456789", "오늘의 할 일을 차근차근 정리해 보세요. 0123456789")}</p></div><footer><span>✓ {tr("Saved to your account", "계정에 저장됩니다")}</span><button type="button" onClick={() => setFontPickerOpen(false)}>{tr("Done", "완료")}</button></footer></section></div>}
    ...
    <section className="workspace-main">
      ...
      {view === "mindmap" && <div className="content mindmap-content">
        ...
        <section className="mindmap-shell">
          <header className={`mindmap-toolbar ${linkingMindmapNodeId ? "is-linking" : ""}`}><div><button onClick={() => zoomMindmap(mindmapViewport.zoom * .85)} aria-label={tr("Zoom out", "축소")}>−</button><output>{Math.round(mindmapViewport.zoom * 100)}%</output><button onClick={() => zoomMindmap(mindmapViewport.zoom * 1.15)} aria-label={tr("Zoom in", "확대")}>＋</button><button className="mindmap-fit-button" onClick={fitMindmap}>⌗ {tr("Fit map", "전체 보기")}</button></div><p>{linkingMindmapNodeId ? tr("Link mode: click anywhere on another idea · click the source again to cancel", "연결 모드: 다른 아이디어를 누르세요 · 취소하려면 시작 아이디어를 다시 누르세요") : tr("Use + Link on an idea to connect · click a line to disconnect", "아이디어의 + 연결로 이어 주세요 · 선을 누르면 연결을 해제할 수 있어요")}</p><button className="mindmap-save-button" disabled={!mindmapDirty && mindmapSaveState !== "error"} onClick={() => void saveMindmap()}>✓ {tr("Save now", "지금 저장")}</button></header>
          <div className="mindmap-workspace">
            ...
            <div ref={mindmapCanvasRef} className={`mindmap-canvas ${mindmapGestureRef.current?.mode === "pan" ? "is-panning" : ""} ${linkingMindmapNodeId ? "is-linking" : ""}`} onPointerDown={beginMindmapPan} onPointerMove={moveMindmapPointer} onPointerUp={finishMindmapPointer} onPointerCancel={finishMindmapPointer} onWheel={handleMindmapWheel} onClick={event => { if (!(event.target as HTMLElement).closest(".mindmap-node,.mindmap-edge") && !mindmapDidMoveRef.current) { setSelectedMindmapNodeId(null); setSelectedMindmapEdgeId(null); setLinkingMindmapNodeId(null); } }} onDoubleClick={event => { if (!(event.target as HTMLElement).closest(".mindmap-node")) addMindmapNode(event.clientX, event.clientY); }} aria-label={tr("Infinite mindmap canvas", "무한 마인드맵 캔버스")}>
              <div className="mindmap-world" style={{transform:`translate(${mindmapViewport.x}px, ${mindmapViewport.y}px) scale(${mindmapViewport.zoom})`}}>
                {mindmapEdges.map(edge => { const from = mindmapNodes.find(node => node.id === edge.from); const to = mindmapNodes.find(node => node.id === edge.to); if (!from || !to) return null; const x1 = from.x + 110; const y1 = from.y + 65; const x2 = to.x + 110; const y2 = to.y + 65; const distance = Math.hypot(x2 - x1, y2 - y1); const angle = Math.atan2(y2 - y1, x2 - x1) * 180 / Math.PI; return <div className={`mindmap-edge ${selectedMindmapEdgeId === edge.id ? "selected" : ""}`} style={{left:x1,top:y1,width:distance,transform:`rotate(${angle}deg)`}} key={edge.id}><button className="mindmap-edge-hit" onPointerDown={event => event.stopPropagation()} onClick={event => { event.stopPropagation(); setSelectedMindmapEdgeId(edge.id); setSelectedMindmapNodeId(null); setLinkingMindmapNodeId(null); }} aria-label={tr(`Select connection from ${from.title} to ${to.title}`, `${from.title}에서 ${to.title} 연결 선택`)} /><i />{selectedMindmapEdgeId === edge.id && <button className="mindmap-edge-remove" style={{transform:`translate(-50%,-50%) rotate(${-angle}deg)`}} onPointerDown={event => event.stopPropagation()} onClick={event => { event.stopPropagation(); disconnectMindmapEdge(edge.id); }} aria-label={tr(`Disconnect ${from.title} and ${to.title}`, `${from.title}와 ${to.title} 연결 해제`)}>× <span>{tr("Disconnect", "연결 해제")}</span></button>}</div>; })}
                {mindmapNodes.map(node => <article className={`mindmap-node ${node.color} ${node.imagePath ? "with-image" : ""} ${selectedMindmapNodeId === node.id ? "selected" : ""} ${linkingMindmapNodeId === node.id ? "linking" : ""} ${linkingMindmapNodeId && linkingMindmapNodeId !== node.id ? "link-target" : ""}`} style={{left:node.x,top:node.y}} onPointerDown={event => beginMindmapNodeDrag(event, node)} onClick={() => { if (mindmapDidMoveRef.current) return; if (linkingMindmapNodeId && linkingMindmapNodeId !== node.id) connectMindmapNode(node.id); else { setSelectedMindmapNodeId(node.id); setSelectedMindmapEdgeId(null); } }} tabIndex={0} key={node.id}><header><i /><span>{node.title.slice(0,1).toUpperCase()}</span><button className="mindmap-link-button" onClick={event => { event.stopPropagation(); connectMindmapNode(node.id); }} disabled={Boolean(session) && !currentPermissions?.edit_cards} aria-label={linkingMindmapNodeId && linkingMindmapNodeId !== node.id ? tr(`Connect to ${node.title}`, `${node.title}에 연결`) : linkingMindmapNodeId === node.id ? tr("Cancel connection", "연결 취소") : tr(`Start a connection from ${node.title}`, `${node.title}에서 연결 시작`)}>{linkingMindmapNodeId === node.id ? `× ${tr("Cancel", "취소")}` : linkingMindmapNodeId ? `＋ ${tr("Connect here", "여기에 연결")}` : `＋ ${tr("Link", "연결")}`}</button></header>{node.imagePath && mindmapImageUrls[node.imagePath] && <img src={mindmapImageUrls[node.imagePath]} alt="" draggable={false} />}<h3>{node.title}</h3><p>{node.note}</p><footer><span>{mindmapEdges.filter(edge => edge.from === node.id || edge.to === node.id).length} {tr("links", "개 연결")}</span>{node.imagePath && <span>▧ {tr("image", "이미지")}</span>}<b>⋮⋮</b></footer></article>)}
              </div>
            ...
            <aside className={`mindmap-inspector ${selectedMindmapNode ? "open" : ""}`}>{selectedMindmapNode ? <><header><div><small>{tr("SELECTED IDEA", "선택한 아이디어")}</small><b>{tr("Node details", "노드 상세")}</b></div><button onClick={() => setSelectedMindmapNodeId(null)}>×</button></header>{selectedMindmapNode.imagePath && mindmapImageUrls[selectedMindmapNode.imagePath] && <figure><img src={mindmapImageUrls[selectedMindmapNode.imagePath]} alt={selectedMindmapNode.title} /><button onClick={() => void removeMindmapImage(selectedMindmapNode)}>× {tr("Remove image", "이미지 삭제")}</button></figure>}<label>{tr("Title", "제목")}<input value={selectedMindmapNode.title} maxLength={100} disabled={Boolean(session) && !currentPermissions?.edit_cards} onChange={event => updateMindmapNode(selectedMindmapNode.id, { title: event.target.value })} /></label><label>{tr("Thought or note", "생각 또는 메모")}<textarea value={selectedMindmapNode.note} maxLength={400} disabled={Boolean(session) && !currentPermissions?.edit_cards} onChange={event => updateMindmapNode(selectedMindmapNode.id, { note: event.target.value })} /></label><fieldset><legend>{tr("Color", "색상")}</legend>{(["violet","mint","coral","blue","amber"] as MindmapNode["color"][]).map(color => <button className={color === selectedMindmapNode.color ? "active" : ""} onClick={() => updateMindmapNode(selectedMindmapNode.id, { color })} aria-label={color} disabled={Boolean(session) && !currentPermissions?.edit_cards} key={color}><i className={color} /></button>)}</fieldset><div className="mindmap-node-tools"><button onClick={() => mindmapImageInputRef.current?.click()} disabled={mindmapImageUploading || Boolean(session) && !currentPermissions?.edit_cards}>▧ {mindmapImageUploading ? tr("Uploading…", "업로드 중…") : selectedMindmapNode.imagePath ? tr("Replace image", "이미지 교체") : tr("Attach image", "이미지 첨부")}</button><button onClick={() => createCardFromMindmap(selectedMindmapNode)} disabled={Boolean(session) && !currentPermissions?.edit_cards}>▤ {tr("Create card", "카드 만들기")}</button></div><section className="mindmap-connections"><header><div><small>{tr("CONNECTIONS", "연결")}</small><b>{selectedMindmapConnections.length}</b></div><button className={linkingMindmapNodeId === selectedMindmapNode.id ? "active" : ""} onClick={() => connectMindmapNode(selectedMindmapNode.id)}>{linkingMindmapNodeId === selectedMindmapNode.id ? `× ${tr("Cancel", "취소")}` : `＋ ${tr("Add link", "연결 추가")}`}</button></header>{selectedMindmapConnections.length > 0 ? <div>{selectedMindmapConnections.map(({ edge, node }) => <article key={edge.id}><span>{node?.title.slice(0,1).toUpperCase()}</span><b>{node?.title}</b><button onClick={() => disconnectMindmapEdge(edge.id)} disabled={Boolean(session) && !currentPermissions?.edit_cards} aria-label={tr(`Disconnect ${node?.title}`, `${node?.title} 연결 해제`)}>×</button></article>)}</div> : <p>{tr("No connections yet. Add a link, then click another idea.", "아직 연결이 없습니다. 연결 추가 후 다른 아이디어를 누르세요.")}</p>}</section><div className="mindmap-inspector-actions"><button className={linkingMindmapNodeId === selectedMindmapNode.id ? "active" : ""} onClick={() => connectMindmapNode(selectedMindmapNode.id)}>＋ {linkingMindmapNodeId === selectedMindmapNode.id ? tr("Cancel connection", "연결 취소") : tr("Connect idea", "아이디어 연결")}</button><button className="danger-button" onClick={() => deleteMindmapNode(selectedMindmapNode.id)}>× {tr("Delete", "삭제")}</button></div><footer><small>{tr("Shortcuts", "단축키")}</small><span><kbd>⌘</kbd><kbd>↵</kbd> {tr("new idea", "새 아이디어")}</span><span><kbd>⌫</kbd> {tr("delete selected node or line", "선택한 노드 또는 선 삭제")}</span><span><kbd>⌘</kbd><kbd>0</kbd> {tr("fit map", "전체 보기")}</span></footer></> : <div className="mindmap-inspector-empty"><span>⌘</span><b>{tr("Select an idea", "아이디어 선택")}</b><p>{tr("Click a node to edit it, attach an image, or create a production card.", "노드를 선택하여 수정하거나 이미지를 첨부하고 프로덕션 카드를 만드세요.")}</p></div>}</aside><input ref={mindmapImageInputRef} className="mindmap-image-input" type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={uploadMindmapImage} />
          </div>
        </section>
      </div>}
