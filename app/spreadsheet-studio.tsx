...
import { ChangeEvent, Dispatch, KeyboardEvent as ReactKeyboardEvent, PointerEvent as ReactPointerEvent, SetStateAction, useEffect, useMemo, useRef, useState } from "react";
...
export default function SpreadsheetStudio({ documents, setDocuments, session, activeWorkspaceId, accountName, accountEmail, canEdit, language, setToast, onCreateCard }: Props) {
  ...
  const [selectedRange, setSelectedRange] = useState<CellRange>({ start: { row: 0, column: 0 }, end: { row: 0, column: 0 } });
  ...
  const selectionDragRef = useRef<{ pointerId: number; start: CellPosition } | null>(null);
  ...
  const selectedCellIds = useMemo(() => { const ids: string[] = []; for (let row = Math.min(selectedRange.start.row, selectedRange.end.row); row <= Math.max(selectedRange.start.row, selectedRange.end.row); row += 1) for (let column = Math.min(selectedRange.start.column, selectedRange.end.column); column <= Math.max(selectedRange.start.column, selectedRange.end.column); column += 1) ids.push(cellId(row, column)); return ids; }, [selectedRange]);
  const selectionLabel = selectedCellIds.length > 1 ? `${selectedCellIds[0]}:${selectedCellIds[selectedCellIds.length - 1]}` : selectedCell;
  ...
  useEffect(() => {
    ...
    setTitle(activeDocument.title); setData(parsed ?? { ...starter, cells: {} }); setSelectedCell("A1"); setSelectedRange({ start: { row: 0, column: 0 }, end: { row: 0, column: 0 } }); setDirty(false); setSaveState("saved");
  }, [activeDocument]);
  ...
  useEffect(() => { const finishSelection = () => { selectionDragRef.current = null; }; window.addEventListener("pointerup", finishSelection); window.addEventListener("pointercancel", finishSelection); return () => { window.removeEventListener("pointerup", finishSelection); window.removeEventListener("pointercancel", finishSelection); }; }, []);
  ...
  async function createSheet() {
    ...
    try {
      ...
      const saved = mapDocument(result.document); loadedIdRef.current = saved.id; setDocuments(current => [saved, ...current]); setActiveId(saved.id); setTitle(saved.title); setData(starter); setSelectedCell("A1"); setSelectedRange({ start: { row: 0, column: 0 }, end: { row: 0, column: 0 } }); setDirty(false); setSaveState("saved");
  ...
  function updateCell(id: string, value: string) { setData(current => { const position = parseCellId(id), rowHeights = { ...current.rowHeights }; if (position && value && !value.startsWith("=")) { const width = current.columnWidths[columnName(position.column)] ?? 126, charactersPerLine = Math.max(4, Math.floor((width - 16) / 7)), lines = value.split("\n").reduce((total, line) => total + Math.max(1, Math.ceil(line.length / charactersPerLine)), 0), neededHeight = Math.min(300, Math.max(34, lines * 18 + 10)), row = String(position.row + 1); if (neededHeight > (rowHeights[row] ?? 34)) rowHeights[row] = neededHeight; } return { ...current, cells: { ...current.cells, [id]: value }, rowHeights }; }); markDirty(); }
  function updateSelectedStyle(patch: Partial<CellStyle>) { setData(current => { const styles = { ...current.styles }; selectedCellIds.forEach(id => { styles[id] = { ...(styles[id] ?? {}), ...patch }; }); return { ...current, styles }; }); markDirty(); }
  function clearSelectedStyle() { setData(current => { const styles = { ...current.styles }; selectedCellIds.forEach(id => delete styles[id]); return { ...current, styles }; }); markDirty(); }
  function setSelectedColumnWidth(width: number) { if (!Number.isFinite(width)) return; setData(current => ({ ...current, columnWidths: { ...current.columnWidths, [selectedColumn]: Math.round(Math.min(600, Math.max(40, width))) } })); markDirty(); }
  function setSelectedRowHeight(height: number) { if (!Number.isFinite(height)) return; const row = String(selectedPosition.row + 1); setData(current => ({ ...current, rowHeights: { ...current.rowHeights, [row]: Math.round(Math.min(300, Math.max(20, height))) } })); markDirty(); }
  function moveSelection(rowDelta: number, columnDelta: number) { const row = Math.min(data.rowCount - 1, Math.max(0, selectedPosition.row + rowDelta)), column = Math.min(data.columnCount - 1, Math.max(0, selectedPosition.column + columnDelta)), position = { row, column }, id = cellId(row, column); setSelectedCell(id); setSelectedRange({ start: position, end: position }); window.setTimeout(() => document.querySelector<HTMLTextAreaElement>(`[data-cell="${id}"]`)?.focus(), 0); }
  function handleCellKeyDown(event: ReactKeyboardEvent<HTMLTextAreaElement>) {
  ...
  function deleteSelectedRow() {
    ...
    const nextPosition = { row: Math.max(0, selectedPosition.row - 1), column: selectedPosition.column }; setData(current => { const cells: Record<string,string> = {}, styles: Record<string,CellStyle> = {}, rowHeights: Record<string,number> = {}; for (let row = 0; row < current.rowCount; row += 1) for (let column = 0; column < current.columnCount; column += 1) { if (row === selectedPosition.row) continue; const target = cellId(row > selectedPosition.row ? row - 1 : row, column), source = cellId(row, column), value = current.cells[source]; if (value !== undefined) cells[target] = value; if (current.styles[source]) styles[target] = current.styles[source]; } Object.entries(current.rowHeights).forEach(([row, height]) => { const index = Number(row) - 1; if (index !== selectedPosition.row) rowHeights[String((index > selectedPosition.row ? index - 1 : index) + 1)] = height; }); return { ...current, rowCount: current.rowCount - 1, cells, styles, rowHeights }; }); setSelectedCell(cellId(nextPosition.row, nextPosition.column)); setSelectedRange({ start: nextPosition, end: nextPosition }); markDirty();
  }
  function deleteSelectedColumn() {
    ...
    const nextPosition = { row: selectedPosition.row, column: Math.max(0, selectedPosition.column - 1) }; setData(current => { const cells: Record<string,string> = {}, styles: Record<string,CellStyle> = {}, columnWidths: Record<string,number> = {}; for (let row = 0; row < current.rowCount; row += 1) for (let column = 0; column < current.columnCount; column += 1) { if (column === selectedPosition.column) continue; const target = cellId(row, column > selectedPosition.column ? column - 1 : column), source = cellId(row, column), value = current.cells[source]; if (value !== undefined) cells[target] = value; if (current.styles[source]) styles[target] = current.styles[source]; } Object.entries(current.columnWidths).forEach(([column, width]) => { const index = column.charCodeAt(0) - 65; if (index !== selectedPosition.column) columnWidths[columnName(index > selectedPosition.column ? index - 1 : index)] = width; }); return { ...current, columnCount: current.columnCount - 1, cells, styles, columnWidths }; }); setSelectedCell(cellId(nextPosition.row, nextPosition.column)); setSelectedRange({ start: nextPosition, end: nextPosition }); markDirty();
  }
  function clearSelection() { setData(current => { const cells = { ...current.cells }; selectedCellIds.forEach(id => delete cells[id]); return { ...current, cells }; }); markDirty(); }
  function beginCellSelection(event: ReactPointerEvent<HTMLTableCellElement>, position: CellPosition) { const start = event.shiftKey ? selectedRange.start : position; selectionDragRef.current = { pointerId: event.pointerId, start }; setSelectedCell(cellId(position.row, position.column)); setSelectedRange({ start, end: position }); }
  function extendCellSelection(event: ReactPointerEvent<HTMLTableCellElement>, position: CellPosition) { const drag = selectionDragRef.current; if (!drag || event.buttons !== 1) return; event.preventDefault(); setSelectedCell(cellId(position.row, position.column)); setSelectedRange({ start: drag.start, end: position }); }
  function exportCsv() {
    ...
    {!session ? <section className="spreadsheet-signin"><span>▦</span><h3>{tr("Sign in to use spreadsheets", "스프레드시트를 사용하려면 로그인하세요")}</h3><p>{tr("Sheets are private and available only to members of this workspace.", "시트는 비공개이며 이 워크스페이스 멤버만 사용할 수 있습니다.")}</p></section> : <section className="spreadsheet-shell">
      ...
      <div className="spreadsheet-workspace">
        ...
        <div className="spreadsheet-formatbar">
          <div className="spreadsheet-format-group"><small>{tr("CELL", "셀")}</small><button className={selectedStyle.bold ? "active" : ""} disabled={!canEdit} onClick={() => updateSelectedStyle({ bold: !selectedStyle.bold })} aria-label={tr("Bold", "굵게")}><b>B</b></button>{(["left", "center", "right"] as CellAlign[]).map(align => <button className={selectedStyle.align === align ? "active" : ""} disabled={!canEdit} onClick={() => updateSelectedStyle({ align })} aria-label={tr(`${align} align`, `${align} 정렬`)} key={align}>{align === "left" ? "≡" : align === "center" ? "≣" : "≡"}</button>)}<label className="spreadsheet-color-control"><span>{tr("Fill", "채우기")}</span><input type="color" value={selectedStyle.fill ?? "#ffffff"} disabled={!canEdit} onChange={event => updateSelectedStyle({ fill: event.target.value })} /></label><label className="spreadsheet-color-control text"><span>{tr("Text", "글자")}</span><input type="color" value={selectedStyle.text ?? "#343b37"} disabled={!canEdit} onChange={event => updateSelectedStyle({ text: event.target.value })} /></label><label className="spreadsheet-border-control"><span>{tr("Border", "테두리")}</span><select value={selectedStyle.border ?? "thin"} disabled={!canEdit} onChange={event => updateSelectedStyle({ border: event.target.value as CellBorder })}><option value="none">{tr("None", "없음")}</option><option value="thin">{tr("Thin", "얇게")}</option><option value="strong">{tr("Strong", "굵게")}</option></select></label><button className="spreadsheet-format-reset" disabled={!canEdit || !selectedCellIds.some(id => Object.keys(data.styles[id] ?? {}).length)} onClick={clearSelectedStyle}>↺ {tr("Reset", "초기화")}</button></div>
          <div className="spreadsheet-size-group"><small>{tr("SIZE", "크기")}</small><label><span>{tr(`Column ${selectedColumn}`, `${selectedColumn}열`)}</span><input type="range" min="40" max="600" value={selectedColumnWidth} disabled={!canEdit} onChange={event => setSelectedColumnWidth(Number(event.target.value))} /><span className="spreadsheet-pixel-input"><input key={`column-${selectedColumn}-${selectedColumnWidth}`} type="number" min="40" max="600" step="1" defaultValue={selectedColumnWidth} disabled={!canEdit} aria-label={tr(`Column ${selectedColumn} width in pixels`, `${selectedColumn}열 너비(픽셀)`)} onBlur={event => setSelectedColumnWidth(Number(event.target.value))} onKeyDown={event => { if (event.key === "Enter") event.currentTarget.blur(); }} /><i>px</i></span></label><label><span>{tr(`Row ${selectedPosition.row + 1}`, `${selectedPosition.row + 1}행`)}</span><input type="range" min="20" max="300" value={selectedRowHeight} disabled={!canEdit} onChange={event => setSelectedRowHeight(Number(event.target.value))} /><span className="spreadsheet-pixel-input"><input key={`row-${selectedPosition.row}-${selectedRowHeight}`} type="number" min="20" max="300" step="1" defaultValue={selectedRowHeight} disabled={!canEdit} aria-label={tr(`Row ${selectedPosition.row + 1} height in pixels`, `${selectedPosition.row + 1}행 높이(픽셀)`)} onBlur={event => setSelectedRowHeight(Number(event.target.value))} onKeyDown={event => { if (event.key === "Enter") event.currentTarget.blur(); }} /><i>px</i></span></label></div>
        </div>
        <div className="spreadsheet-formula"><label title={selectionLabel}>{selectionLabel}</label><span>ƒx</span><input value={data.cells[selectedCell] ?? ""} disabled={!canEdit} placeholder={tr("Enter a value or formula, for example =SUM(G2:G10)", "값 또는 수식을 입력하세요. 예: =SUM(G2:G10)")} onChange={event => updateCell(selectedCell, event.target.value)} onKeyDown={event => { if (event.key === "Enter") moveSelection(1, 0); }} /><button disabled={!canEdit || !selectedCellIds.some(id => data.cells[id])} onClick={clearSelection}>×</button></div>
        <div className="spreadsheet-grid-wrap" ref={gridRef}><table className="spreadsheet-grid"><thead><tr><th className="spreadsheet-corner" />{Array.from({ length: data.columnCount }, (_, column) => { const width = data.columnWidths[columnName(column)] ?? 126; return <th className={selectedPosition.column === column ? "selected" : ""} style={{ minWidth: width, width }} key={columnName(column)}>{columnName(column)}</th>; })}</tr></thead><tbody>{Array.from({ length: data.rowCount }, (_, row) => { const height = data.rowHeights[String(row + 1)] ?? 34; return <tr className={row < data.frozenRows ? "frozen" : ""} style={{ height }} key={row}><th className={selectedPosition.row === row ? "selected" : ""} style={{ height }}>{row + 1}</th>{Array.from({ length: data.columnCount }, (_, column) => { const id = cellId(row, column), raw = data.cells[id] ?? "", display = editingCell === id ? raw : evaluateCell(id, data.cells), cellStyle = data.styles[id] ?? {}, width = data.columnWidths[columnName(column)] ?? 126, border = cellStyle.border ?? "thin", inRange = selectedCellIds.includes(id); return <td className={`${selectedCell === id ? "selected active-cell" : ""} ${inRange ? "range-selected" : ""} ${raw.startsWith("=") ? "formula" : ""}`} style={{ minWidth: width, width, height, background: cellStyle.fill, color: cellStyle.text, fontWeight: cellStyle.bold ? 800 : undefined, textAlign: cellStyle.align, borderColor: border === "none" ? "transparent" : undefined, boxShadow: border === "strong" ? "inset 0 0 0 2px #59645f" : undefined }} onPointerDown={event => beginCellSelection(event, { row, column })} onPointerEnter={event => extendCellSelection(event, { row, column })} key={id}><textarea rows={1} data-cell={id} value={display} readOnly={!canEdit} onFocus={() => { setSelectedCell(id); setEditingCell(id); }} onBlur={() => setEditingCell(null)} onChange={event => updateCell(id, event.target.value)} onKeyDown={handleCellKeyDown} /></td>; })}</tr>; })}</tbody></table></div>
        <footer className="spreadsheet-status"><span>{data.rowCount} {tr("rows", "행")} × {data.columnCount} {tr("columns", "열")}</span><span>{selectionLabel}: <b>{selectedCellIds.length > 1 ? tr(`${selectedCellIds.length} cells selected`, `${selectedCellIds.length}개 셀 선택`) : evaluateCell(selectedCell, data.cells) || tr("Empty", "비어 있음")}</b></span><span>{tr("Drag to select · Shift-click to extend", "드래그하여 선택 · Shift-클릭으로 확장")}</span></footer>
