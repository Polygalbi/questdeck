...
type SheetData = { version: 1; rowCount: number; columnCount: number; cells: Record<string, string>; frozenRows: number };
type Props = {
...
function parse(content: string): SheetData | null {
  ...
  try {
    ...
    return { version: 1, rowCount: Math.min(200, Math.max(5, Number(value.rowCount) || 18)), columnCount: Math.min(26, Math.max(3, Number(value.columnCount) || 7)), frozenRows: Math.min(5, Math.max(0, Number(value.frozenRows) || 0)), cells: value.cells as Record<string, string> };
  } catch { return null; }
}
...
export default function SpreadsheetStudio({ documents, setDocuments, session, activeWorkspaceId, accountName, accountEmail, canEdit, language, setToast, onCreateCard }: Props) {
  ...
  function deleteSelectedRow() {
    ...
    setData(current => { const cells: Record<string,string> = {}; for (let row = 0; row < current.rowCount; row += 1) for (let column = 0; column < current.columnCount; column += 1) { if (row === selectedPosition.row) continue; const value = current.cells[cellId(row, column)]; if (value !== undefined) cells[cellId(row > selectedPosition.row ? row - 1 : row, column)] = value; } return { ...current, rowCount: current.rowCount - 1, cells }; }); setSelectedCell(cellId(Math.max(0, selectedPosition.row - 1), selectedPosition.column)); markDirty();
  }
  function deleteSelectedColumn() {
    ...
    setData(current => { const cells: Record<string,string> = {}; for (let row = 0; row < current.rowCount; row += 1) for (let column = 0; column < current.columnCount; column += 1) { if (column === selectedPosition.column) continue; const value = current.cells[cellId(row, column)]; if (value !== undefined) cells[cellId(row, column > selectedPosition.column ? column - 1 : column)] = value; } return { ...current, columnCount: current.columnCount - 1, cells }; }); setSelectedCell(cellId(selectedPosition.row, Math.max(0, selectedPosition.column - 1))); markDirty();
  }
  ...
  async function importCsv(event: ChangeEvent<HTMLInputElement>) {
    ...
    try { const rows = parseCsv(await file.text()).slice(0, 200), columnCount = Math.min(26, Math.max(3, ...rows.map(row => row.length))), cells: Record<string,string> = {}; rows.forEach((row, rowIndex) => row.slice(0, 26).forEach((value, columnIndex) => { if (value) cells[cellId(rowIndex, columnIndex)] = value; })); setData({ version: 1, rowCount: Math.max(5, rows.length), columnCount, frozenRows: 1, cells }); setTitle(file.name.replace(/\.csv$/i, "")); setSelectedCell("A1"); markDirty(); setToast(tr("CSV imported", "CSV를 가져왔습니다")); }
  ...
  return <div className="content spreadsheet-content">
    ...
    {!session ? <section className="spreadsheet-signin"><span>▦</span><h3>{tr("Sign in to use spreadsheets", "스프레드시트를 사용하려면 로그인하세요")}</h3><p>{tr("Sheets are private and available only to members of this workspace.", "시트는 비공개이며 이 워크스페이스 멤버만 사용할 수 있습니다.")}</p></section> : <section className="spreadsheet-shell">
      ...
      <div className="spreadsheet-workspace">
        ...
        <div className="spreadsheet-grid-wrap" ref={gridRef}><table className="spreadsheet-grid"><thead><tr><th className="spreadsheet-corner" />{Array.from({ length: data.columnCount }, (_, column) => <th className={selectedPosition.column === column ? "selected" : ""} key={columnName(column)}>{columnName(column)}</th>)}</tr></thead><tbody>{Array.from({ length: data.rowCount }, (_, row) => <tr className={row < data.frozenRows ? "frozen" : ""} key={row}><th className={selectedPosition.row === row ? "selected" : ""}>{row + 1}</th>{Array.from({ length: data.columnCount }, (_, column) => { const id = cellId(row, column), raw = data.cells[id] ?? "", display = editingCell === id ? raw : evaluateCell(id, data.cells); return <td className={`${selectedCell === id ? "selected" : ""} ${raw.startsWith("=") ? "formula" : ""}`} key={id}><input data-cell={id} value={display} readOnly={!canEdit} onFocus={() => { setSelectedCell(id); setEditingCell(id); }} onBlur={() => setEditingCell(null)} onChange={event => updateCell(id, event.target.value)} onKeyDown={handleCellKeyDown} /></td>; })}</tr>)}</tbody></table></div>
